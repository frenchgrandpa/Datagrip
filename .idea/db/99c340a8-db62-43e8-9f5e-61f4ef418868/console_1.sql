select * from users where id =28378
...
where buyer_id=28378
order by id desc
select * from item_specs where id = 1987

select * from balance_transactions where user_id = 22196 and reason > -1 order by id desc
select * from users where steam_id = '76561199222747117'
...
select * from item_transactions where seller_id = 23017 or buyer_id = 23017 order by id desc
...
select * from items where asset_id = '26563777681'
select * from items where id = 17653405
...
select * from items where owner_id = 108
--                       and app_id = 252490
                      and in_use = true
                      and id not in (
        select item_id
        from sell_listings
        where sell_listings.owner_id = 108
          and status = 0
    )
...
select * from sell_listings where status = 0 and item_id is null
...
inner join tracking_info ti on ti.id = users.tracking_info_id
where last_login_date < date '1 apr 2022'
and banned = false
and users.id > 109 and users.id not in (169,
